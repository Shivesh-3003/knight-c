...
<Info>
  **Getting testnet USYC:** USYC on Arc Testnet is permissioned. Developers must
  be allowlisted before interacting with the contract. To request access, open a
  ticket with [Circle Support](https://support.circle.com/) and include your Arc
  Testnet wallet address for allowlisting. Once approved (typically in 24-48
  hours), you will receive a small test USYC allocation, which you can use for
  testing on the Arc Testnet.
...
[StableFX](https://developers.circle.com/stablefx) is an institutional-grade
...
> Arc's stable fee model uses USDC as gas to provide predictable, low-cost, and auditable onchain transaction fees.
...
> Arc's block explorers to view transactions, verify contracts, and analyze onchain data using Etherscan-compatible APIs.
